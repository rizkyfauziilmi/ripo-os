...
export const useWindowStore = create<WindowState>((set) => ({
  ...
  openWindow: (appName: string) => {
    set((state) => {
      const existingWindow = state.windows.find((window) => window.appName === appName)
      if (existingWindow) {
        if (existingWindow.isOpen) {
          toast.warning(`${appName} is already open`, {
            description: 'You can only have one instance of this app running at a time.',
            position: 'top-center',
        ...
        return {
          windows: state.windows.map((window) =>
            window.id === existingWindow.id ? { ...window, isOpen: true } : window
          ),
        }
      } else {
