    ...
    <div className="flex items-center w-fit flex-1">
      <div className="ml-2 flex flex-col justify-center gap-4 px-2 rounded-md py-4 bg-background/80 border-[1.5px]">
        {Array.from(icon).map(([key, IconComponent]) => {
          const currentWindow = windows.find((window) => window.appName === key)
          return (
            <div key={key} className="relative">
              {currentWindow && <AppIndicator isActive={currentWindow.isOpen} />}
              <DockAppIcon app={key} icon={IconComponent} onClick={() => openWindow(key)} />
            </div>
          )
        })}
      </div>
